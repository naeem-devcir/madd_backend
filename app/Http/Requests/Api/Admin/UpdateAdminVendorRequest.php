...
class UpdateAdminVendorRequest extends FormRequest
{
    ...
    {
        $vendor = $this->route('id');
        $vendorModel = is_numeric($vendor)
            ? \App\Models\Vendor\Vendor::find($vendor)
            : \App\Models\Vendor\Vendor::where('uuid', $vendor)->first();

        $userId = $vendorModel?->user_id;
        ...
        return [
            ...
            'company_slug' => [
                ...
                'max:255',
                'regex:/^[a-z0-9-]+$/',
                Rule::unique('vendors', 'company_slug')->ignore($vendorId),
            ],
